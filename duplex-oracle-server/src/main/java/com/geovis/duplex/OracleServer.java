...
public class OracleServer {
	...
	private void load(String filePath) throws Exception {
		...
		while(iterator.hasNext()) {
			...
			Iterator<Element> tables = schema.elementIterator("table");
			while(tables.hasNext()) {
				...
				Iterator<Element> filters = table.elementIterator("filter");
				List<FilterModel> filterModels = new ArrayList<>();
				while(filters.hasNext()) {
					Element filter = filters.next();
					String filterTable = filter.attributeValue("table");
					
					Iterator<Element> columns = filter.elementIterator("column");
					while(columns.hasNext()){
						Element column = columns.next();
						String columnName = column.attributeValue("name");
						String method = column.attributeValue("method");
						String value = column.getStringValue();
						FilterModel filterModel = new FilterModel(filterTable, columnName, method, value);
						filterModels.add(filterModel);
					}
				}
				tableModel.setFilterModels(filterModels);
				
				Iterator<Element> fields = table.elementIterator("field");
	...
	public class TableTask extends AbstractTask {
		...
		public void run() {
			...
			try {
				...
				extractor.setTableModel(tableModel);
