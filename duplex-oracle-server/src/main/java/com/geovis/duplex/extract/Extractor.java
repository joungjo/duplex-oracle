...
import com.geovis.duplex.filter.CarrierFilter;
...
public class Extractor implements DataExtract {
	...
	public void startup(){
		...
		List<FilterModel> filterModels = this.tableModel.getFilterModels();
		if (filterModels != null) 
			for (FilterModel filterModel : filterModels) {
				CarrierFilter filter = new CarrierFilter();
				filter.setPush(this.push);
	...
	private String keySQL(String table){
		...
		return "select * from " + subscrpName
				+ " where USERNAME$ != '" + ConnectionFactory.getUser().toUpperCase() + "' and OPERATION$ != 'UU' "
				+ "and OPERATION$ != 'UO'";
	} 
